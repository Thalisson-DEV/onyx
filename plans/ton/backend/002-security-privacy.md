...
- [ ] TON tasks reject missing or mismatched tenant context.
- [ ] ZIP entry, expanded-byte and member limits are enforced.
- [ ] Unknown-size uploads are counted during streaming.
- [ ] SSE exposes stable error information only.
- [ ] Tool/model traces are metadata-only by default.
- [ ] Backend telemetry and browser PostHog/GTM/custom analytics are off by default in TON.
- [ ] Query redirects and window messages use allowlists and source/origin checks.
- [ ] Named tests and typecheck pass.
